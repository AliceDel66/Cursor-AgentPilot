...
本项目是 **工作流手册 + 派发协议**，不是官方文档翻译，也不是泛 AI 工具测评：

- **Cursor 是 cockpit**：输入、理解、拆解、协调、轻量执行。
- **Codex 是 executor**：本地 repo 修改、跑测试、验证、交付总结。
...
4. **选路径派发**：对照 `docs/03-dispatch-design.md` 的 routing matrix（骨架期可先看 `docs/00-project-brief.md` 第 9.2 节），判断这个任务该交给 Cursor 直接改、Codex 执行，还是需要 Claude review。
5. **验收与留档**：执行完成后对照验收标准逐条检查；如果走了 Codex/Claude，用 `templates/review-packet.md` 汇总证据，由你做最终 human gate。
...
| 3 | `docs/03-dispatch-design.md` | 把任务拆成 task package，派发给 Codex / Claude |
...
cursor-agentpilot/
  ...
  docs/
    ...
    03-dispatch-design.md          # Cursor -> Codex/Claude 任务派发设计
